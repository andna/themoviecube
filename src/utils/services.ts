...
export const cubeSides = 6;
export const perPageResults = 9;
export const maximumItems = 1000;
export const maximumPage = Math.floor(maximumItems/perPageResults) + 1;
const pos = { half: 0.505, init: 0};
const halfPi = Math.PI / 2;


export const aux = 0.05; //for beauty purposes
...
export const cubePositions = [
    ...
    { pos: [-pos.half, pos.init, 0],    rot: [0, halfPi*3, 0],  rotTo: {x: 0, y:(-halfPi*3) - aux, z:-aux} as THREE.Euler },
    { pos: [0, pos.half, -pos.init],    rot: [halfPi*3, 0, 0],  rotTo: {x: halfPi - aux, y: 0, z: aux } as THREE.Euler },
    { pos: [0, -pos.half, pos.init],    rot: [halfPi, 0, 0],    rotTo: {x: halfPi*3 - aux, y: 0, z: -aux} as THREE.Euler },
];
...
export const getInfoFromGithubApi = async (searchTerm: string,
    ...
    if(searchTerm){
        ...
        const space = page === 1 ? ' ' : '&';
        await fetch(
            `https://api.github.com/search/users?q=${searchTerm}&per_page=${perPageResults}&page=${page}${space}in:login`)
            .then(response => {
